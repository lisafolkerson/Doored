...
if ( ($deleteselected == '1') && ($deleteassignselected == '1') && current_user_can( $lmm_options[ 'capabilities_delete_others' ]) ) {
...
} else if ( ($deleteselected == '1') && isset($_POST['checkedlayers']) && current_user_can( $lmm_options[ 'capabilities_delete_others' ]) ) {
	...
	$wpdb->query( "DELETE FROM `$table_name_markers` WHERE `layer` IN (" . htmlspecialchars($checked_layers) . ")");
	$wpdb->query( "OPTIMIZE TABLE `$table_name_layers`" );
	$wpdb->query( "OPTIMIZE TABLE `$table_name_markers`" );
	echo '<p><div class="updated" style="padding:10px;">' . sprintf(__('The selected layers (IDs %s) and all assigned markers have been deleted','lmm'), htmlspecialchars($checked_layers)) . '</div>';
	...
	foreach ($_POST['checkedlayers'] as $row){
		$wpdb->query( "UPDATE $table_name_markers SET layer = " . intval($_POST['layer']) . " where layer = " . $row . "");
	}
	$wpdb->query( "DELETE FROM `$table_name_layers` WHERE `id` IN (" . htmlspecialchars($checked_layers) . ")");
	echo '<p><div class="updated" style="padding:10px;">' . sprintf(__('The selected layers (IDs %1$s) have been deleted and the markers have been assigned to %2$s','lmm'), htmlspecialchars($checked_layers), htmlspecialchars($_POST['layer'])) . '</div>';
	...
	$checked_layers_prepared = implode(",", $_POST['checkedlayers']);
	$checked_layers = preg_replace('/[a-z|A-Z| |\=]/', '', $checked_layers_prepared);
	echo '<p><div class="updated" style="padding:10px;">' . __('The selected layers have been duplicated. New layer IDs:','lmm') . ' ';
	...
	<?php if ( (current_user_can( $lmm_options[ 'capabilities_delete_others' ])) || (current_user_can( $lmm_options[ 'capabilities_edit_others' ])) ) { ?>
		...
		<p><input type="radio" id="duplicateselected" name="bulkactions-layers" value="duplicateselected" /> <label for="duplicateselected"><?php _e('duplicate layer only','lmm') ?></label></p>
		<?php if (current_user_can( $lmm_options[ 'capabilities_delete_others' ])) { ?>
		<p><input type="radio" id="deleteselected" name="bulkactions-layers" value="deleteselected" /> <label for="deleteselected"><?php _e('delete layer and assigned markers','lmm') ?></label></p>
		<?php } ?>
		<?php 
		$layerlist = $wpdb->get_results('SELECT * FROM `'.$table_name_layers.'` WHERE `id` > 0 AND `multi_layer_map` = 0', ARRAY_A); 
		if (current_user_can( $lmm_options[ 'capabilities_edit_others' ])) { ?>
			<input type="radio" id="deleteassignselected" name="bulkactions-layers" value="deleteassignselected" /> <label for="deleteassignselected"><?php _e('delete layer and assign markers to the following layer:','lmm') ?></label>
